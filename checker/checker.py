...
external_logger = logging.getLogger("Checker_DEBUG")
...
async def putflag_license(task: PutflagCheckerTaskMessage, client: AsyncClient, db: ChainDB) -> None:   
    ...
    user = await register_user(client=client,logger=None,db=db,chain_id=task.task_chain_id, fileLogger=external_logger)
    shop_item_kwargs =  {
        'data_path': '/frog.png',
        'item_name': item_name,
        'logged_in': True,
...
async def getflag_license(task: GetflagCheckerTaskMessage, client: AsyncClient, db: ChainDB) -> None:
    ...
    await login(client=client,logger=None,db=db,kwargs=login_kwargs,fileLogger=external_logger)
    try:
...
async def exploit_staff(searcher: FlagSearcher, client: AsyncClient, db: ChainDB,) -> None:
    ...
    staff_user = await create_staff_user(client=client,logger=None,db=db,kwargs=staff_kwargs,fileLogger=external_logger)

    login_data = {
        'username':staff_user['username'],
        'password':staff_user['password1'],
    }
    await login(client=client,logger=None,db=db,kwargs=login_data,fileLogger=external_logger)
